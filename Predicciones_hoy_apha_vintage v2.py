...
##########################################################
def insert_hyphen(symbol):
    return symbol[:3] + '-' + symbol[3:]


# Aplicar la función al símbolo
modified_symbol = insert_hyphen(symbol)
print(modified_symbol)
...
def fetch_data(api_key, symbol, interval, outputsize):
    ...
    df = data.rename(columns={'Close': 'close'}).astype(float)
    return df
